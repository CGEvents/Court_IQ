...
export default async function handler(req, res) {
  ...
  try {
    ...
    // NOTE: new Google "AQ" auth keys must be passed via the x-goog-api-key
    // header (not the old ?key= query param), so we use the header below.
    ...
    if (!r.ok) {
      // Surface the real reason in logs so we can see WHY a key is rejected,
      // but still return null so the app falls back to the offline coach.
      let detail = "";
      try { detail = (await r.text()).slice(0, 300); } catch {}
      console.log("Gemini upstream error", r.status, detail);
